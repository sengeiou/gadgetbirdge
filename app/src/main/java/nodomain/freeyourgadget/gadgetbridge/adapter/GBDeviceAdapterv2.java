...
//                                                             });
    ...
    private void exCsv(GBDevice device) throws GBException, IOException {
        long i = 1;

        DeviceCoordinator coordinator = DeviceHelper.getInstance().getCoordinator(device);
//        File myPath = FileUtils.getExternalFilesDir();
//        File myFile = new File(myPath, device.getName() + ".csv");
//        CsvWriter csvWriter = new CsvWriter(new FileWriter(myFile), ',');
...
//                String add[] = {sample.getTimestamp() + "", sample.getSteps() + "", sample.getHeartRate() + ""};
//                csvWriter.writeRecord(add);
                ...
                try {
                    ...
                    Request request = new Request.Builder()
                            // 指定访问的服务器地址
                            .url("http://47.111.180.30:5657/").post(RequestBody2)
                            .build();
